...
class macro_definitions {
  ...
  void add_macro(char* name, char* contents) {
    ...
    macros[nmacros].name     = new char[strlen(name    ) + 1];  strcpy(macros[nmacros].name,     name    );
    macros[nmacros].contents = new char[strlen(contents) + 1];  strcpy(macros[nmacros].contents, contents);
    ++nmacros;
  }
